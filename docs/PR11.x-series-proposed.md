...
outside the admitted surface. Those residuals are then taken up explicitly by
`PR11.9a` / `PR11.9b` and the later concurrency follow-ons, rather than being
left in an unnamed post-freeze bucket. The shared I/O/runtime seam closes
under `PR11.8g.2`.
...
- Each referenced channel gains a forwarding procedure called by `Send`
  to notify the dispatcher.
- The `or delay` arm maps to `select Disp.Await(...) or delay N.N;` —
  a timed entry call on the single dispatcher entry.
- `Try_Receive` is removed from select lowering; it stays for standalone
  `try_receive` statements.
