# v0.3 Syntax: Brace-Delimited Blocks
...
# v0.3 Syntax Proposed: Emitter-Based Container Instantiation
...
# v0.3 Reference: SPARK Container Library Compatibility Analysis

## Purpose

This section documents the compatibility between SPARK's formal container
library (`SPARK.Containers.Formal.*`) and Safe's design restrictions. It
serves as a reference for the container-related proposals in this document.
...
3. **The emitter can use them.** Since D16 applies to Safe source, not emitted
   ...
   basis for the emitter-based container instantiation proposal above.
...
5. **SPARK's containers are not proven internally.** If Safe wants Silver-level
   ...
   need to be written in Safe or verified separately. The SPARK containers
   only provide verified-usage guarantees.
